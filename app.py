...
# --- เพิ่มฟังก์ชันสำหรับสร้าง Pop-up 💅🏻 พุ่งเต็มจอ ---
def celebrate_success():
    # ใช้ JavaScript สร้าง Particle เป็น Emoji 💅🏻 กระจายเต็มจอ
    components.html(
        """
        <div id="celebration" style="position:fixed; top:0; left:0; width:100vw; height:100vh; pointer-events:none; z-index:9999;"></div>
        <script src="https://cdn.jsdelivr.net/npm/canvas-confetti@1.5.1/dist/confetti.browser.min.js"></script>
        <script>
            var end = Date.now() + (3 * 1000); // เล่น 3 วินาที
            var scalar = 3;
        ...
        <div class="fabulous-text">เริ่ดเลยหล่ะ</div>
        """,
        height=0,
    )

# --- โค้ดส่วนประมวลผล Excel (เหมือนเดิม) ---
def process_excel_to_buffer(uploaded_file):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        for branch_name, branch_data in final_list.groupby('Branch', sort=False):
            ...
            apply_styles_to_sheet(writer.sheets[sheet_name], branch_name, store_code, current_date, is_summary=False)
...
# (ฟังก์ชัน apply_styles_to_sheet คงเดิมตามเวอร์ชันก่อนหน้า...)
def apply_styles_to_sheet(ws, branch_name, store_code, current_date, is_summary=False):
    ...
    if not is_summary:
        ...
        for i, h in enumerate(["", "MBL", "BNN"]):
            c = ws.cell(row=s_row, column=5+i, value=h); c.font, c.fill, c.border = f_white, fill_green, border
        for i, label in enumerate(["ตะกร้าใหญ่", "ตะกร้าเล็ก"], 1):
            ws.cell(row=s_row + i, column=5, value=label).font, ws.cell(row=s_row + i, column=5).border = f_header, border
            ws.cell(row=s_row + i, column=6).border = ws.cell(row=s_row + i, column=7).border = border
...
if uploaded_file:
    ...
    if st.button("🚀 ประมวลผลไฟล์", use_container_width=True):
        with st.spinner('กำลังคำนวณ...'):
            st.session_state.excel_bytes = process_excel_to_buffer(uploaded_file)
            st.session_state.processed = True
    
    if st.session_state.processed:
        # เมื่อประมวลผลเสร็จแล้ว แสดง Pop-up เริ่ดๆ
        celebrate_success()
        
        st.success("เริ่ดมาก! จัดการไฟล์เสร็จแล้วค่ะ 💅🏻")
        st.download_button(
            label="📥 ดาวน์โหลดไฟล์ที่เริ่ดที่สุด",
            data=st.session_state.excel_bytes,
            file_name=f"POD_Fabulous_{datetime.now().strftime('%H%M')}.xlsx",
            use_container_width=True,
            on_click=lambda: st.balloons() # แถมลูกโป่งตอนกดโหลดด้วย
        )